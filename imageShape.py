...
class imageShape:
    ...
    def generateShape(self):
        ...
        self.shape = np.zeros(size,np.uint8) # coloca los valores de la imagen en 0
        #self.ind_shapes = np.random.randint(1, 5) # genera un numero entero aleatorio entre 1 y 4
        self.ind_shapes = 4  # genera un numero entero aleatorio entre 1 y 4
    ...
    def whatShape(self,shape):
        ...
        Gray_Image = cv2.cvtColor(shape_copy, cv2.COLOR_BGR2GRAY) # pasa la imagen a escala de grises
        ret, Ibw_shape = cv2.threshold(Gray_Image, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU) # hace la umbralizacion con metodo OTSU
        ...
        #Si el numero de vertices encontrados es 3 entonces retorne Triangle
        if len(points) == 3:
            return "Triangle"

        # Si el numero de vertices encontrados es 4 entonces
        if len(points) == 4:
            area = cv2.contourArea(contours[0]) # calcular el area del contorno encontrado
            horizontal_side = self.width / 2
            vertical_side = self.height / 2
            # compare el area calculada del rectangulo con el area del contorno
            if horizontal_side*vertical_side == area:
                return "Rectangle" # si coincide retornar Rectangule
            else:
                return "Square" # sino retornar Square

        # Si el numero de vertices encontrados es mayor a 4 entonces retornar Circle
        if len(points) > 4:
            return "Circle"